...
Features • Installation • Architecture • Privacy
...
Web Privacy Analyzer is a forensic dashboard designed for security researchers and privacy-conscious users. Unlike standard "what is my IP" sites, this tool performs a dual-layer analysis:
    ...
    Server-Side Interception: Captures raw HTTP request headers, proxy forwarded IPs, and protocol metadata.

    Client-Side Fingerprinting: Executes JavaScript probes to extract hardware details, screen metrics, and WebGL renderer info that can uniquely identify users even behind a VPN.
...
python privacy_analyzer/app.py
